...
def create_distribution_chart(df):
    """Create a distribution chart (histogram) for the first numeric column."""
    numeric_cols = df.select_dtypes(include=['number']).columns
...
def create_auto_chart(df):
    """Automatically select a chart type based on data."""
    numeric_cols = df.select_dtypes(include=['number']).columns
    categorical_cols = df.select_dtypes(include=['object']).columns
    ...
    elif not categorical_cols.empty and not numeric_cols.empty:
        # Bar chart for categorical vs numeric
        return px.bar(df, x=categorical_cols[0], y=numeric_cols[0], title=f"{numeric_cols[0]} by {categorical_cols[0]}")
...
def create_visualization(results):
    if results is not None:
        ...
        if not df.empty:
            ...
            fig = create_auto_chart(df)
            if fig:
                st.plotly_chart(fig, use_container_width=True)
            else:
                st.info("Could not automatically generate a suitable visualization for this data.")